"""
daily_scan.py  —  Full automated daily scan: $0-$1 US penny stocks, 30-point scoring
Runs every day at 2:30 PM (Saudi time).
"""

import os, sys, json, csv, glob, time, datetime
import math
...
# ── Step 1: Clear old cache and parts ─────────────────────────────────────────
def clear_stale():
...
# ── Step 2: Run all chunks sequentially ───────────────────────────────────────
def run_all_chunks():
...
# ── Step 3: Combine parts + build HTML ────────────────────────────────────────
def build_report():
    ...
    for i, r in enumerate(scored, 1):
        ...
        rows_html_parts.append(
            ...
            f'<td style="text-align:right;color:#80cbc4">${price:.4f}</td>'
...
<head>
<meta charset="UTF-8">
<title>US Penny Stock Scan $0-$1 | {DATE_STR}</title>
<style>
...
<body>
<div class="header">
  <h1>&#127482;&#127480; US PENNY STOCK SCAN &mdash; $0 to $1</h1>
...
function sortTable(col) {{
  ...
  table.setAttribute('data-sort-dir', asc ? 'desc' : 'asc');
    ...
    html_path = os.path.join(BASE, f"US_Penny_Scan_{TODAY}.html")
    with open(html_path, 'w', encoding='utf-8') as f:
        f.write(html)
    size_kb = os.path.getsize(html_path) // 1024
    log(f"  HTML report saved: US_Penny_Scan_{TODAY}.html ({size_kb} KB)")
    return html_path

# ── Main ───────────────────────────────────────────────────────────────────────
if __name__ == '__main__':
    log("=" * 60)
    log(f"PENNY SCAN STARTED — {DATE_STR}")
    log("=" * 60)
